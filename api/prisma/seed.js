...
// Fallback if Prisma.SupplierType is missing
const SupplierType = (Prisma && Prisma.SupplierType) ?? {
  PHYSICAL: 'PHYSICAL',
  ONLINE: 'ONLINE',
};// ---- Config ---------------------------------------------------------------
const SUPER_EMAIL = process.env.SUPERADMIN_EMAIL || 'superadmin@example.com';
...
const DEMO_SEED = String(process.env.DEMO_SEED || 'false') === 'true';
const VARIANT_PRODUCT_COUNT = Number(process.env.VARIANT_PRODUCT_COUNT || 50);
// Run tag so reruns append different SKUs. Set SEED_RUN_TAG to pin idempotency.
const RUN_TAG = (process.env.SEED_RUN_TAG || Date.now().toString(36)).toUpperCase();
...
const asDec = (n) => new Prisma.Decimal(n);
...
// ---- Bootstrap: settings + super-admin ------------------------------------
async function ensureCoreTaxSettings() {
  try {
    await prisma.setting.createMany({
      data: [
...
// ---- Ensure Suppliers / Brands / Categories -------------------------------
async function ensureSuppliers() {
  ...
  for (const name of names) {
    let s = await prisma.supplier.findFirst({ where: { name }, select: { id: true, name: true } });
    if (!s) {
      s = await prisma.supplier.create({
        data: {
          name,
          type: chance(0.5) ? SupplierType.PHYSICAL : SupplierType.ONLINE,
          status: 'ACTIVE',
          contactEmail: `${name.toLowerCase().replace(/[^a-z0-9]+/g, '')}@example.com`,
          whatsappPhone: `+2348${randInt(100000000, 999999999)}`,
        },
  ...
  log(`Suppliers ensured: ${out.length}`);
  return out;
}
...
async function ensureBrands() {
  const brandNames = ['DaySpring', 'NaijaTech', 'GreenFarm', 'FitLife', 'UrbanHome', 'SilverPeak'];
  const out = [];
  for (const b of brandNames) {
    const slug = b.toLowerCase();
    const br = await prisma.brand.upsert({
...
// ---- Ensure Attributes (+ options with codes) ------------------------------
async function ensureAttributes() {
  ...
  const attrs = {}; // name -> { id, name, values: [{id,name,code}] }

  for (const def of defs) {
    // attribute (find-or-create by name)
    let a = await prisma.attribute.findFirst({ where: { name: def.name }, select: { id: true, name: true } });
    if (!a) {
      a = await prisma.attribute.create({
        data: { name: def.name, type: 'SELECT', isActive: true },
        select: { id: true, name: true },
      });
    } else {
      await prisma.attribute.update({ where: { id: a.id }, data: { isActive: true } });
    }

    // values (ensure present, ensure code)
    for (const v of def.values) {
      const existing = await prisma.attributeValue.findFirst({
        where: { attributeId: a.id, name: v.name },
        select: { id: true },
      });
      if (!existing) {
        await prisma.attributeValue.create({
          data: { attributeId: a.id, name: v.name, code: v.code, position: 0, isActive: true },
        });
      } else {
        await prisma.attributeValue.update({
          where: { id: existing.id },
          data: { code: v.code, isActive: true },
        });
      }
    }

    // load back with values
    const values = await prisma.attributeValue.findMany({
      where: { attributeId: a.id },
      select: { id: true, name: true, code: true },
      orderBy: { position: 'asc' },
    });
    attrs[def.name] = { id: a.id, name: def.name, values };
  }

  log('Attributes ensured:', Object.keys(attrs).join(', '));
  return attrs;
}
...
async function seedVariantProducts(superAdminId, suppliers, categories, brands, attrs) {
  const t0 = Date.now();
  ...
  for (let i = 1; i <= VARIANT_PRODUCT_COUNT; i++) {
    const brand = sample(brands);
    const cat = sample(categories);
    const sup = sample(suppliers);

    const baseSku = `VR-${RUN_TAG}-${String(i).padStart(4, '0')}`;
    const title = `Variant Product ${RUN_TAG} #${i}`;
    const basePrice = randInt(4000, 120000);
    ...
    const product = await prisma.product.create({
      data: {
        title,
        description: 'Seeded product with variants. Replace with real catalog content before launch.',
        price: asDec(basePrice),
        sku: baseSku,
        inStock: true,
        status: 'LIVE',
        imagesJson: productImages(`${RUN_TAG}-${i}`),
        availableQty: randInt(10, 30),
        communicationCost: asDec(50),
        supplier: { connect: { id: sup.id } },
        category: { connect: { id: cat.id } },
        brand: { connect: { id: brand.id } },
        owner: { connect: { id: superAdminId } },
    ...
    // choose allowed options (ProductAttributeOption superset)
    const chooseSome = (arr, min, max) => {
      const n = Math.max(min, Math.min(max, arr.length));
      const count = randInt(min, n);
      return shuffle(arr).slice(0, count);
    };

    const allowedByAttr = {};
    const colorVals = chooseSome(color.values, 3, Math.min(5, color.values.length));
    const sizeVals = chooseSome(size.values, 2, Math.min(4, size.values.length));
    const weightVals = chooseSome(weight.values, 2, weight.values.length);
    const volumeVals = chooseSome(volume.values, 2, volume.values.length);

    const paoRows = [];

    for (const v of colorVals) {
      (allowedByAttr[color.id] ||= []).push(v.id);
      paoRows.push({ productId: product.id, attributeId: color.id, valueId: v.id });
    }
    for (const v of sizeVals) {
      (allowedByAttr[size.id] ||= []).push(v.id);
      paoRows.push({ productId: product.id, attributeId: size.id, valueId: v.id });
    }
    for (const v of weightVals) {
      (allowedByAttr[weight.id] ||= []).push(v.id);
      paoRows.push({ productId: product.id, attributeId: weight.id, valueId: v.id });
    }
    for (const v of volumeVals) {
      (allowedByAttr[volume.id] ||= []).push(v.id);
      paoRows.push({ productId: product.id, attributeId: volume.id, valueId: v.id });
    }

    if (paoRows.length) {
      await prisma.productAttributeOption.createMany({ data: paoRows, skipDuplicates: true });
    }

    // Build variants (mix of 1..4 attributes)
    const combos = shuffle([
      ...
      [color, size, weight],
      [color, size, volume],
      [color, size, weight, volume],
    ]).filter((attrsList) => attrsList.every((a) => (allowedByAttr[a.id] || []).length > 0));

    const variantTarget = randInt(3, 8);
    const usedSkus = new Set();

    for (const combo of combos.slice(0, variantTarget)) {
      const chosenValues = combo.map((a) => {
        const allowedIds = allowedByAttr[a.id];
        const vId = sample(allowedIds);
        const v = (a.values.find((x) => x.id === vId)) || { code: 'X' };
        const code = (v.code || 'X').toUpperCase().replace(/\W+/g, '');
        return { attrId: a.id, valueId: vId, code };
      });

      const sku = `${baseSku}-${chosenValues.map((cv) => cv.code).join('-')}`;
      ...
      const variant = await prisma.productVariant.create({
        data: {
          ...
          sku,
          price: bump ? asDec(Math.max(500, basePrice + bump)) : null,
          inStock: true,
          imagesJson: productImages(`${RUN_TAG}-${sku}`),
          availableQty: randInt(10, 30),
        },
        select: { id: true },
      });
      ...
      const pvos = chosenValues.map((cv) => ({
        variantId: variant.id,
        attributeId: cv.attrId,
        valueId: cv.valueId,
        priceBump: chance(0.3) && bump ? asDec(randInt(50, bump)) : null,
      }));
      await prisma.productVariantOption.createMany({ data: pvos, skipDuplicates: true });
    }

    // Supplier offers: 2–5 per product (mix of variant-bound and product-wide)
    const offersToMake = randInt(2, 5);
    const productVariants = await prisma.productVariant.findMany({
      where: { productId: product.id },
      select: { id: true },
    });
    const offerKeys = new Set(); // supplierId::variantId|null

    for (let k = 0; k < offersToMake; k++) {
      const supplier = sample(suppliers);
      const useVariant = productVariants.length > 0 && chance(0.7);
      const variantPick = useVariant ? sample(productVariants).id : null;
      const key = `${supplier.id}::${variantPick || 'NULL'}`;
      if (offerKeys.has(key)) { k--; continue; }
      offerKeys.add(key);
      ...
      await prisma.supplierOffer.create({
        data: {
          ...
          price: supplierOfferFromRetail(basePrice * (0.95 + Math.random() * 0.15)),
          currency: 'NGN',
          inStock: true,
          availableQty: randInt(10, 30),
    ...
    if (i % 10 === 0) {
      log(`Progress: ${i}/${VARIANT_PRODUCT_COUNT} products (variants: ${createdVariants}, offers: ${createdOffers})…`);
    }
  }

  log(`✅ Variant batch ${RUN_TAG} done in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
  log(`   Products created: ${createdProducts}`);
  log(`   Variants created: ${createdVariants}`);
  log(`   SupplierOffers created: ${createdOffers}`);
}

// ---- Optional tiny demo ----------------------------------------------------
async function seedDemoCatalog(superAdminId) {
  log('Seeding DEMO catalog (categories/brands/products)…');

  const cats = await ensureCategories();
  const brands = await ensureBrands();
  const sups = await ensureSuppliers();
...
async function main() {
  ...
  await seedVariantProducts(superId, suppliers, categories, brands, attrs);
