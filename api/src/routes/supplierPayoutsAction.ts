...
router.post("/purchase-orders/:purchaseOrderId/release", requireAuth, async (req: any, res: Response) => {
  try {
    if (!isSupplier(req.user?.role)) return res.status(403).json({ error: "Forbidden" });

    const supplier = await getSupplierForUser(String(req.user?.id));
    if (!supplier?.id) return res.status(403).json({ error: "Supplier access required" });

    const po = await prisma.purchaseOrder.findUnique({
      where: { id: String(req.params.purchaseOrderId) },
      select: { supplierId: true },
    });
    if (!po || po.supplierId !== supplier.id) return res.status(403).json({ error: "Forbidden" });

    const out = await paySupplierForPurchaseOrder(String(req.params.purchaseOrderId), { id: req.user?.id, role: req.user?.role });
