...
router.post("/purchase-orders/:poId/delivery-otp/request", requireAuth, async (req: Request, res: Response) => {
    ...
    if (!["SHIPPED", "OUT_FOR_DELIVERY", "DELIVERED"].includes(poStatus)) {
        return res.status(400).json({
            error: "Delivery OTP can only be requested when PO is SHIPPED / OUT_FOR_DELIVERY / DELIVERED",
        });
    }
    ...
    if (alreadyVerified?.id) {
        return res.json({ ok: true, alreadyVerified: true, verifiedAt: alreadyVerified.verifiedAt });
    }
