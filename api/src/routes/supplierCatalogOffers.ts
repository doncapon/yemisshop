...
router.get("/products", requireAuth, async (req: any, res) => {
    ...
    if (!isSupplier(role)) return res.status(403).json({ error: "Supplier access required" });

    const s = await getSupplierForUser(userId);
    if (!s?.id) return res.status(403).json({ error: "Supplier not found" });

    const supplierId = s.id; // ✅ FIX: correct field name
    ...
    const where: any = {
        isDeleted: false,
        status: "LIVE",
        // other suppliers (or null supplierId)
    ...
    const [items, total] = await Promise.all([
        prisma.product.findMany({
            ...
            select: {
                ...
                // ✅ canonical field in your schema
                retailPrice: true,
                ...
                // existing offers by THIS supplier (so UI can show/edit status quickly)
    ...
    // - Ensure imagesJson is always []
    // - Provide backward-compatible aliases `price` for product/variant
    const normalized = (items ?? []).map((p: any) => {
        ...
        const normVariants = variants.map((v: any) => ({
            ...
            price: v.retailPrice ?? null, // alias for UI
        }));
 ...
 *  - they own it, OR
 *  - they have ever offered it
 *  - OR it's a LIVE catalog product (not theirs) so they can create offers for it
 */
router.get("/:id", requireAuth, async (req: any, res) => {
    ...
    const s = await getSupplierForUser(userId);
    if (!s?.id) return res.status(403).json({ error: "Supplier profile not found for this user" });
    ...
    const p = await prisma.product.findFirst({
        where: {
            ...
            OR: [
                ...
                // allow viewing LIVE catalog product (not owned by this supplier)
                {
                    AND: [
                        { status: "LIVE" as any },
    ...
    const variantOfferIds = (p as any).ProductVariant
        ?.map((vv: any) => vv?.supplierVariantOffers?.[0]?.id)
        .filter(Boolean) ?? [];
...
router.put("/offers/base", requireAuth, async (req: any, res) => {
    ...
    // If it's a brand new offer, you can decide:
    // - either create immediately (common)
    // - or require approval on first publish (stricter)
    // We'll do common: create immediately.
    if (!existing) {
        const created = await prisma.supplierProductOffer.create({
            ...
            select: { id: true, supplierId: true, productId: true, basePrice: true, availableQty: true, leadDays: true, isActive: true, inStock: true, currency: true, updatedAt: true },
        });

        return res.json({ data: created, meta: { reviewQueued: false } });
    }

    // Determine immediate vs review deltas
    ...
    const result = await prisma.$transaction(async (tx: any) => {
        ...
        // Queue review changes
        let queued: any = null;
        if (reviewChanged) {
            queued = await tx.supplierOfferChangeRequest.upsert({
                where: {
                    // uses @@unique([supplierProductOfferId, status])
                    uniq_pending_base_offer_change: { supplierProductOfferId: existing.id, status: "PENDING" },
                } as any,
                create: {
                    supplierId,
                    productId: body.productId,
                ...
                select: { id: true, status: true, proposedPatch: true, requestedAt: true },
            });
        }
...
router.put("/offers/variant", requireAuth, async (req: any, res) => {
    ...
    const v = await prisma.productVariant.findUnique({
        ...
        select: { id: true, productId: true, product: { select: { id: true, isDeleted: true, status: true, supplierId: true } } },
    });
    ...
    // New variant offer: create immediately (common)
    if (!existing) {
        const created = await prisma.supplierVariantOffer.create({
            ...
            select: { id: true, supplierId: true, productId: true, variantId: true, unitPrice: true, availableQty: true, leadDays: true, isActive: true, inStock: true, currency: true, updatedAt: true },
        });
        return res.json({ data: created, meta: { reviewQueued: false } });
    }
    ...
    const result = await prisma.$transaction(async (tx: any) => {
        ...
        if (reviewChanged) {
            queued = await tx.supplierOfferChangeRequest.upsert({
                where: {
                    uniq_pending_variant_offer_change: { supplierVariantOfferId: existing.id, status: "PENDING" },
                } as any,
                create: {
                    supplierId,
                    productId: body.productId,
...
router.delete("/offers/variant/:id", requireAuth, async (req: any, res) => {
    ...
    if (!found || found.supplierId !== supplierId) return res.status(404).json({ error: "Not found" });
