...
export default function ForgotPassword() {
  ...
  const inputBase =
    "w-full rounded-xl border bg-surface pl-10 pr-3 py-3 " +
    "text-[16px] sm:text-sm text-ink placeholder:text-ink-soft transition " +
    "focus:outline-none focus:ring-4 focus:ring-primary-100 focus:border-primary-400";

  return (
    <SiteLayout>
      <div className="min-h-[100dvh] relative overflow-hidden bg-gradient-to-b from-primary-50/60 via-bg-soft to-bg-soft">
        {/* subtle blobs */}
        <div className="pointer-events-none absolute -top-24 -left-24 size-64 rounded-full bg-primary-400/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-24 -right-24 size-72 rounded-full bg-accent-400/20 blur-3xl" />

        <div className="relative mx-auto w-full max-w-md px-4 sm:px-6 py-8 sm:py-12">
          {/* Header */}
          ...
          <form
            onSubmit={submit}
            noValidate
            className="mt-6 rounded-2xl border border-white/60 bg-white/80 backdrop-blur
            ...
            {(msg || err) && (
              <div className="space-y-2">
                {msg && (
                  <div
                    role="status"
                    className="text-sm rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-800 px-3 py-2"
                  >
                    {msg}
                  </div>
                )}
                {err && (
                  <div
                    role="alert"
                    className="text-sm rounded-xl border border-danger/20 bg-danger/10 text-danger px-3 py-2"
                  >
                    {err}
                  </div>
                )}
              ...
              <div className="relative group">
                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-ink-soft">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path
                      d="M4 6h16a1 1 0 0 1 1 1v.2l-9 6-9-6V7a1 1 0 0 1 1-1Z"
                      stroke="currentColor"
                      strokeWidth="1.5"
                    />
                    <path
                      d="M21 8.5V17a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V8.5l9 6 9-6Z"
                      stroke="currentColor"
                      strokeWidth="1.5"
                    />
                  </svg>
