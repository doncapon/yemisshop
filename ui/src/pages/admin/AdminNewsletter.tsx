...
export default function AdminNewsletterPage() {
  ...
  const mutation = useMutation<SendNewsletterResult, any, SendNewsletterPayload>({
    ...
    onSuccess: (data) => {
      ...
      const msg = data.dryRun
        ? `Dry run complete. Found ${data.totalFound} active subscriber(s).`
        : `Newsletter sent to ${data.totalSent} subscriber(s).`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For real sends, use a simple confirm dialog
    if (!dryRun) {
      const ok = window.confirm(
        `Send this newsletter to all active subscribers${limitNum ? ` (up to ${limitNum})` : ""}?`
      );
      if (!ok) return;
    }
  ...
  if (!isAdmin) {
    return (
      <div className="px-3 sm:px-4 md:px-6 py-4 md:py-6 max-w-3xl mx-auto">
        <div className="rounded-2xl border bg-white shadow-sm p-4 sm:p-5">
          <div className="flex items-center gap-2 text-amber-700 mb-2">
            <AlertCircle className="h-4 w-4" />
            <h1 className="text-sm sm:text-base font-semibold">Admins only</h1>
          </div>
          <p className="text-xs sm:text-sm text-ink-soft">
            The newsletter broadcast tool is restricted to admin users.
          </p>
        </div>
      </div>
    );
      ...
      <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-ink-soft mt-1 max-w-xl">
            Send an occasional update to all active newsletter subscribers. Use{" "}
            <span className="font-medium">dry run</span> first to see how many
            subscribers would receive the email.
          </p>
        </div>
        ...
        <div className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-[11px] text-amber-800 border border-amber-200">
          ...
          <span>Admins only · internal tool</span>
        </div>
      </div>
