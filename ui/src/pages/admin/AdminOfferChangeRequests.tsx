...
import { CheckCircle2, XCircle, RefreshCcw, Search, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AdminOfferChangeRequests() {
  ...
  const listQ = useQuery({
    ...
    enabled: !!token,
    queryFn: async () => {
      const res = await api.get(`/api/admin/offer-change-requests?status=${tab}`, {
    ...
    <SiteLayout>
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-zinc-900">Offer change approvals</h1>
            <p className="text-sm text-zinc-600 mt-1">
              Approve or reject supplier changes (price / lead-days / active status). Stock changes are applied immediately.
            </p>
          </div>
            ...
            {(["PENDING", "APPROVED", "REJECTED"] as const).map((t) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold ${
                  tab === t ? "bg-zinc-900 text-white" : "bg-white text-zinc-800 hover:bg-black/5"
                }`}
              >
                {t}
              </button>
        ...
        {listQ.isLoading && (
          <div className="rounded-2xl border bg-white p-4 text-sm text-zinc-600">Loading…</div>
        )}
        ...
        {!listQ.isLoading && filtered.length === 0 && (
          <div className="rounded-2xl border bg-white p-4 text-sm text-zinc-600">No items.</div>
        )}
        ...
        <div className="space-y-3">
          {filtered.map((x) => {
            ...
            return (
              <div key={x.id} className="rounded-2xl border bg-white/90 shadow-sm overflow-hidden">
                <div className="p-4 flex flex-col md:flex-row md:items-center gap-3">
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-zinc-900">
                      {productTitle} <span className="text-zinc-400 font-normal">({sku})</span>
                    </div>
                    <div className="text-xs text-zinc-600 mt-1">
                      Supplier: <b className="text-zinc-900">{supplierName}</b> • Scope:{" "}
                    ...
                    <div className="text-xs text-zinc-700 mt-2">
                      ...
                      <span className="font-mono">{JSON.stringify(x.proposedPatch ?? {})}</span>
                    </div>
                  </div>
