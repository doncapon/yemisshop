...
type AdminProduct = {
  ...
  price: number | string;
  status: string;
  imagesJson?: string[] | string; // can be array or joined string from API
  createdAt?: string;
  isDelete?: boolean;
  ownerId?: boolean;
  availableQty?: number;             // <- will be overridden (derived)
  ...
  sku?: string | null;
  inStock?: boolean;                 // <- may be overridden (derived)
  variants?: any[];
  variantCount?: number;
...
// Normalize different backend fields into a single availableQty number
const getOfferAvailableQty = (o: any) =>
  toInt(o?.availableQty ?? o?.available ?? o?.qty ?? o?.stock, 0);

// Preset filter type (moved to top-level, used across component)
type FilterPreset =
  | "all"
  | "no-offer"
  | "live"
...
// keep statusFromPreset at top-level too
...
// Derive inStock strictly from quantity (and active flag)
const deriveOfferInStock = (o: any) =>
  (o?.isActive ?? true) && getOfferAvailableQty(o) > 0;
...
export function ManageProducts({
  ...
  const offersSummaryQ = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fallback: per-product endpoints
      if (!all) {
          ...
          for (const u of perAttempts) {
            try {
              ...
              if (Array.isArray(arr)) {
                // ensure productId is present on each record
                per.push(...arr.map((o: any) => ({ ...o, productId: o.productId || pid })));
                break;
              }
            } catch { /* next per-product url */ }
          }
        }
        all = per;
      }
      ...
      // Build per-product summary using robust normalization
      ...
      for (const o of offers) {
        ...
        const availableQty = availOf(o); // <-- robust
        if (!byProduct[pid]) {
      ...
      // Strict: inStock iff totalAvailable > 0
  ...
  // 🔸 DERIVED rows with availableQty + inStock from SupplierOffers
  const rowsWithDerived: AdminProduct[] = React.useMemo(() => {
    const summary = offersSummaryQ.data || {};
    return (rows || []).map((p) => {
      const s = summary[p.id];
      if (!s) return { ...p, availableQty: 0, inStock: p.inStock === true && (p.availableQty ?? 0) > 0 };
      return {
        ...p,
        availableQty: s.totalAvailable,     // <-- derived sum of active offers
        inStock: s.inStock,                 // <-- derived strictly from totalAvailable > 0
      };
    });
  ...
  const getStatus = (p: any): EffectiveStatus => (p?.isDeleted ? "ARCHIVED" : p?.status ?? "PENDING");

  function primaryActionForRow(p: any): RowAction {
    const eff = getStatus(p);

    // availability strictly from offersSummary (derived)
  ...
  // Has-orders map for delete vs soft-delete decisions
  ...
  }, [variantAttrIds, variantValueIds, attrsQ.data]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const filteredRows = React.useMemo(() => {
    ...
    function isAvailableVariantAware(pId: string, p: any) {
      ...
      return p.inStock === true; // fallback if summary missing
    }
    ...
    return rowsWithDerived.filter((p) => {
      switch (preset) {
        case "no-offer":
          return !hasAnyOffer(p.id);
        case "live":
          return p.status === "LIVE";
        case "published-with-offer":
          return p.status === "PUBLISHED" && hasAnyOffer(p.id);
        case "published-no-offer":
          return p.status === "PUBLISHED" && !hasAnyOffer(p.id);
        case "published-with-active":
          return p.status === "PUBLISHED" && hasActiveOffer(p.id);
  ...
  const displayRows = React.useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      switch (sort.key) {
        ...
        case "price":
          ...
          break;
        case "avail":
          res = cmpNum(a?.availableQty ?? getAvail(a), b?.availableQty ?? getAvail(b));
          break;
        case "stock":
    ...
    return arr;
  }, [filteredRows, sort, offersSummaryQ.data]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const saveOrCreate = async () => {
    ...
    if (editingId) {
        ...
        { id: editingId, ...payload },
        {
          onSuccess: async (res) => {
            ...
            if ((payload as any)?.variants?.length) {
              const variantsPersisted =
                Array.isArray((res as any)?.variants) && (res as any).variants.length > 0;
          ...
          onError: (e) =>
            openModal({ title: "Products", message: getHttpErrorMessage(e, "Update failed") }),
        }
      );
    } else {
      createM.mutate(payload, {
        ...
        onError: (e) =>
          openModal({ title: "Products", message: getHttpErrorMessage(e, "Create failed") }),
      });
    }
  };
  ...
  function urlList(): string[] {
    return parseUrlList(pending.imageUrls);
  }
  ...
  function submitEdit(pId: string, intent: "save" | "submitForReview" | "approvePublished" | "movePending") {
    const sourceList = displayRows; // use list with derived fields applied
    const draft = editPendings[pId];
    const source = draft ?? sourceList.find((r: any) => r.id === pId) ?? rowsWithDerived.find((r: any) => r.id === pId);
    if (!source) return;

    if (intent === "approvePublished") {
      const avail = (source.availableQty ?? 0) > 0 || (offersSummaryQ.data?.[pId]?.inStock ?? (source.inStock !== false));
      if (!avail) {
        openModal({
    ...
    // NOTE: availableQty cache lives on the server; here we display derived sum.
    // We intentionally do NOT PATCH availableQty from the UI form.
      ...
      <div id="create-form" className="grid gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          <input
            ...
            placeholder="Title"
            value={pending.title}
            onChange={(e) => setPending((d) => ({ ...d, title: e.target.value }))}
          />
          <input
            className="border rounded-lg px-3 py-2"
            placeholder="Price"
            inputMode="decimal"
            value={pending.price}
            onChange={(e) => setPending((d) => ({ ...d, price: e.target.value }))}
          />
          <input
            className="border rounded-lg px-3 py-2"
            placeholder="Base SKU"
            value={pending.sku}
            onChange={(e) => setPending((d) => ({ ...d, sku: e.target.value }))}
          />
          <select
            className="border rounded-lg px-3 py-2"
            value={pending.status}
            onChange={(e) => setPending((d) => ({ ...d, status: e.target.value }))}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
          <select
            ...
            onChange={(e) => setPending((d) => ({ ...d, categoryId: e.target.value }))}
          >
          ...
          <select
            ...
            onChange={(e) => setPending((d) => ({ ...d, brandId: e.target.value }))}
          >
          ...
          <select
            ...
            onChange={(e) => setPending((d) => ({ ...d, supplierId: e.target.value }))}
          >
        ...
        <div className="rounded-2xl border border-white/60 bg-white/70 backdrop-blur p-4 md:p-5 shadow-[0_6px_30px_rgba(0,0,0,0.06)]">
          ...
          <div className="flex items-center gap-3">
            ...
            {!!files.length && <span className="text-xs text-ink-soft">{files.length} file{files.length === 1 ? "" : "s"} selected</span>}
                      ...
                      <div key={`u:${u}:${i}`} className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1 flex gap-2">
                            <button className="px-2 py-1 text-xs rounded border" onClick={() => moveUrl(i, -1)} disabled={i === 0} title="Move up">
                              ↑
                            </button>
                            <button
                              className="px-2 py-1 text-xs rounded border"
                              onClick={() => moveUrl(i, +1)}
                              disabled={i === urlPreviews.length - 1}
                              title="Move down"
                            >
                              ↓
                            </button>
                            <button className="ml-auto px-2 py-1 text-xs rounded bg-rose-600 text-white" onClick={() => removeUrlAt(i)} title="Remove URL">
                              Remove
                            </button>
                          </div>
                        </div>
                  ...
                  <div className="p-3 space-y-3">
                    {filePreviews.map(({ f, url }, i) => (
                      <div key={`f:${url}:${i}`} className="flex items-start gap-3">
                        <div className="w-24 h-16 rounded border overflow-hidden bg-zinc-50 shrink-0">
                          <img src={url} alt="File preview" className="h-full w-full object-cover" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-medium truncate">{f.name}</div>
                          <div className="text-[11px] text-zinc-500">{(f.size / 1024).toFixed(0)} KB • {f.type || "image/*"}</div>
                          <div className="mt-2 flex gap-2">
                            ...
                            <button className="px-2 py-1 text-xs rounded border" onClick={() => moveFile(i, -1)} disabled={i === 0} title="Move up">
                              ↑
                            </button>
                            <button
                              className="px-2 py-1 text-xs rounded border"
                            ...
                            <button className="ml-auto px-2 py-1 text-xs rounded bg-rose-600 text-white" onClick={() => removeFileAt(i)} title="Remove file">
                              Remove
                            </button>
                          </div>
                        </div>
        ...
        <div className="rounded-2xl border bg-white p-4 md:p-5">
          ...
          {!attrsQ.isLoading && (attrsQ.data || []).length === 0 && <div className="text-sm text-zinc-500">No attributes available.</div>}
        ...
        <div className="rounded-2xl border bg-white p-4 md:p-5">
          ...
          <p className="text-xs text-zinc-600 mb-2">Choose up to two SELECT attributes as variant axes, pick values, then edit SKU suffix / price bumps.</p>
            ...
            {selectableAttrs.map((a: any) => {
              ...
              return (
                <label key={a.id} className="inline-flex items-center gap-2 border rounded-lg px-3 py-1.5 cursor-pointer">
                  <input type="checkbox" checked={checked} onChange={() => toggleVariantAttr(a.id)} disabled={!checked && variantAttrIds.length >= 2} />
                  <span>{a.name}</span>
                </label>
              );
            })}
              ...
              return (
                <div key={aid}>
                  ...
                  <select
                    ...
                    onChange={(e) => setVariantValues(aid, Array.from(e.target.selectedOptions).map((o) => o.value))}
                  >
                    {(a?.values || []).map((v: any) => (
                      <option key={v.id} value={v.id}>
                        {v.name}
                    ...
                    <tr key={r.key}>
                      ...
                      <td className="px-3 py-2">
                        <input
                          ...
                          value={r.skuSuffix}
                          onChange={(e) =>
                            setVariantRows((rows) => rows.map((rr) => (rr.key === r.key ? { ...rr, skuSuffix: e.target.value } : rr)))
                          }
                        />
                      </td>
                      <td className="px-3 py-2">
                        <input
                          ...
                          value={r.priceBump}
                          onChange={(e) =>
                            setVariantRows((rows) => rows.map((rr) => (rr.key === r.key ? { ...rr, priceBump: e.target.value } : rr)))
                          }
                          placeholder="+0"
                        />
                      </td>
      ...
      <div className="flex gap-2 items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="pl-9 pr-3 py-2 rounded-xl border bg-white w-full"
          />
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-full border text-xs ${
              preset === key ? "bg-zinc-900 text-white" : "bg-white hover:bg-zinc-50"
            }`}
          >
            {label}
          </button>
              ...
              displayRows.map((p: any) => {
                ...
                const d =
                  ...
                  ({
                    ...
                    status: p.status,
                    communicationCost:
                      p.communicationCost != null ? String(p.communicationCost) : "",
                  } as EditPending);

                const stockCell = (() => {
                  const inStock = p.inStock === true; // derived already in rowsWithDerived
                  return inStock ? (
                        ...
                        {offersSummaryQ.isLoading ? (
                          ...
                          <span className="inline-flex items-center gap-1">
                            ...
                            <span className="text-xs text-zinc-500">
                              ({(offersSummaryQ.data?.[p.id]?.activeOffers ?? 0)} offer{(offersSummaryQ.data?.[p.id]?.activeOffers ?? 0) === 1 ? "" : "s"})
                            </span>
                          </span>
                        )}
                      </td>
                            ...
                            (() => {
                              ...
                              return (
                                <button
                                  ...
                                  disabled={
                                    deleteM.isPending || restoreM.isPending || action.disabled
                                  }
                                  title={action.title}
                                >
                                  {action.label}
                        ...
                        <td colSpan={isSuper ? 7 : 6} className="px-3 py-3">
                          <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
                            <input
                              ...
                              placeholder="Title"
                              value={d?.title || ""}
                              onChange={(e) => changeEdit(p.id, { title: e.target.value })}
                            />
                            <input
                              className="border rounded-lg px-3 py-2"
                              placeholder="Price"
                              inputMode="decimal"
                              value={d?.price || ""}
                              onChange={(e) => changeEdit(p.id, { price: e.target.value })}
                            />
                            <input
                              className="border rounded-lg px-3 py-2"
                              placeholder="SKU"
                              value={d?.sku || ""}
                              onChange={(e) => changeEdit(p.id, { sku: e.target.value })}
                            />

                            <select
                              className="border rounded-lg px-3 py-2"
                              value={d?.categoryId || ""}
                              onChange={(e) => changeEdit(p.id, { categoryId: e.target.value })}
                            >
                            ...
                            <select
                              className="border rounded-lg px-3 py-2"
                              value={d?.brandId || ""}
                              onChange={(e) => changeEdit(p.id, { brandId: e.target.value })}
                            >
                              <option value="">— Brand —</option>
                              {brandsQ.data?.map((b) => (
                                <option key={b.id} value={b.id}>
                            ...
                            <select
                              className="border rounded-lg px-3 py-2"
                              value={d?.supplierId || ""}
                              onChange={(e) => changeEdit(p.id, { supplierId: e.target.value })}
                            >
                              <option value="">— Supplier —</option>
                              {suppliersQ.data?.map((s) => (
                                <option key={s.id} value={s.id}>
                            ...
                            {isSuper && (
                              <select
                                ...
                                onChange={(e) => changeEdit(p.id, { status: e.target.value })}
                              >
                                <option value="PENDING">PENDING</option>
                                <option value="PUBLISHED">PUBLISHED</option>
                              </select>
                            ...
                            <div className="md:col-span-6 flex items-center justify-end gap-2">
                              ...
                              {isAdmin && (
                                <button
                                  onClick={() => submitEdit(p.id, "submitForReview")}
                                  className="px-3 py-2 rounded-lg bg-amber-600 text-white"
                                  title="Set status to PENDING for approval"
                                >
                                  Submit for Review
                                </button>
                              )}
